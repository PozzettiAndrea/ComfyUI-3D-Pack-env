/* gsplat v1.2.9 -- vendored, with one local patch.
 *
 * Source : https://cdn.jsdelivr.net/npm/gsplat@1.2.9/dist/index.es.js
 * sha256 : bd124c4103dc83e7d9fc9f56e396347e846413f40fe10e4bf8174283de649cc3   (of the pristine upstream file)
 * Upstream: https://github.com/dylanebert/gsplat.js  (MIT)
 *
 * Vendored rather than loaded from a CDN so the viewer works offline, cannot
 * drift under us (the import map previously said gsplat@latest), and adds no
 * third-party request to a page that renders local model output.
 *
 * The ONLY change is credentials: "omit" -> "same-origin" in the loader fetch;
 * search for "PATCHED" below. Re-apply it when bumping the version.
 *
 * .mjs, not .js, is deliberate: ComfyUI auto-imports every *.js under a pack's
 * web dir into the shared main realm. This file is iframe-only, and the .mjs
 * extension keeps it out of that glob.
 */
class R {
...
async function rt(E, t) {
  const n = await fetch(E, {
    mode: "cors",
    // PATCHED (was "omit"). ComfyUI is often served behind an authenticating
    // proxy -- Cloudflare Access, a reverse proxy with basic auth, a tunnel.
    // "omit" drops the session cookie even on a same-origin request, so the
    // proxy sees an anonymous request, 302s to its login origin, and the
    // browser's CORS check on that cross-origin redirect turns it into an
    // opaque "TypeError: Failed to fetch". Same-origin is what three.js'
    // FileLoader uses, which is why mesh previews worked and splats did not.
