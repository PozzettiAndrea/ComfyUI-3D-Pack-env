...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : PointCloud, spatial_lr_scale : float = 1):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().to(comfy.model_management.get_torch_device())
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().to(comfy.model_management.get_torch_device()))
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().to(comfy.model_management.get_torch_device())
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0

        print("Number of points at initialisation : ", fused_point_cloud.shape[0])

        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().to(comfy.model_management.get_torch_device())), 0.0000001)
