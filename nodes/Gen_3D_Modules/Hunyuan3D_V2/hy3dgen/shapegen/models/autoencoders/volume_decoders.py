...
def extract_near_surface_volume_fn(input_tensor: torch.Tensor, alpha: float):
    ...
    # add the offset and handle invalid values
    val = input_tensor + alpha
    valid_mask = val > -9000  # assume -9000 marks an invalid value

    # improved neighbor lookup (keeps dimensions consistent)
    def get_neighbor(t, shift, axis):
        """Shift along the given axis while keeping dimensions consistent"""
        if shift == 0:
            return t.clone()

        # determine the padding axis (input [D, D, D] maps to the z, y, x axes)
        pad_dims = [0, 0, 0, 0, 0, 0]  # format: [x_front, x_back, y_front, y_back, z_front, z_back]

        # set padding according to the axis
        ...
        # perform padding (add batch and channel dims to suit F.pad)
        padded = F.pad(t.unsqueeze(0).unsqueeze(0), pad_dims[::-1], mode='replicate')  # reverse the order to suit F.pad

        # build the dynamic slice index
        slice_dims = [slice(None)] * 3  # initialize to a full slice
    ...
    # get neighbors in each direction (keeping dimensions consistent)
    ...
    # handle invalid boundary values (use where to keep dimensions consistent)
