...
class ImageDreamDiffusion:
    def __init__(
        ...
        self.input_branch_size = input_branch_size
        self.model = model
    ...
    def i2i(
        ...
        with torch.no_grad():
            ...
            if offset_noise:
                ...
                x_T = x_T.repeat(1, input_branch_size, 1, 1)
            ...
            all_samples = []
            for one_sample in samples_ddim:
                x_sample = model.decode_first_stage(one_sample)
                x_sample = torch.clamp((x_sample + 1.0) / 2.0, min=0.0, max=1.0)
                x_sample = x_sample.permute(0, 2, 3, 1) # (N, H, W, 3) in [0, 1]
                all_samples.extend(list(x_sample))
    ...
    def diffuse(self, t, negative_prompt, ip, n_test=2):
        ...
        for _ in range(n_test):
            img = self.i2i(
                ...
                input_branch_size=self.input_branch_size,
                ddim_eta=0.0,
                dtype=self.dtype,
                device=self.device,
                camera=self.camera,
