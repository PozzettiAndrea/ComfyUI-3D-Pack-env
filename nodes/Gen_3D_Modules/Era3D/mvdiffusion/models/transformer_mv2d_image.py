...
class BasicMVTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.cd_attention_last:
            ...
            self.norm_joint_last = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else ops.LayerNorm(dim)
        ...
        if self.cd_attention_mid:
            ...
            self.norm_joint_mid = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else ops.LayerNorm(dim)
...
class MVAttnProcessor:
    ...
    def __call__(
        ...
        # SDPA instead of get_attention_scores + bmm.
        #
        # get_attention_scores materialises the full B*H x M x M score matrix
        # through baddbmm, which is what OOMs here:
        #     CUDA out of memory. Tried to allocate 7.50 GiB
        # SDPA computes the same thing without ever holding that matrix.
        #
