...
def test_both_sampling_versions():
    ...
    if v1_zeros > v2_zeros:
        print(f"[OK] Version 2 is BETTER (fewer zero artifacts: {v2_zeros} vs {v1_zeros})")
        recommended_version = 2
    elif v2_zeros > v1_zeros:
        print(f"[X] Version 1 is better (fewer zero artifacts: {v1_zeros} vs {v2_zeros})")
        recommended_version = 1  
    else:
        print(f"[=] Both versions have similar zero padding: {v1_zeros}")
