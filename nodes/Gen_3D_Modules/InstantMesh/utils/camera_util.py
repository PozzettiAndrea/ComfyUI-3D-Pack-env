...
def oribt_camera_poses_to_input_cameras(azimuths, elevations, batch_size=1, radius=4.0, fov=30.0):
    """
        Args:
            azimuths (list)
            elevations (list)
    ...
    """
    # (0, 180/-180, 0) -> (0, 180, 360)
    azimuths = [360 + angle if angle < 0 else angle for angle in azimuths]
    ...
    Ks = FOV_to_intrinsics(fov).unsqueeze(0).repeat(azimuths.shape[0], 1, 1).float().flatten(-2)
